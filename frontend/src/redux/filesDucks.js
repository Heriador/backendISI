...
export const getFilesAction = (userid) => async (dispatch,getState)=> {
    try {
        const res =  await axios.get(`https://catalogo-onlineapi.herokuapp.com/api/files/all/${userid}`);
        dispatch({
...
export const getFileAction = (id) => async (dispatch, getState) =>{
    try {
        const res = await axios.get(`https://catalogo-onlineapi.herokuapp.com/api/files/${id}`);
        dispatch({
            type: GET_FILE_EXITO,
            payload: res.data
        })
...
export const createFileAction = (data) => async (dispatch, getState) =>{
    try {
        await axios({
            method: 'post',
            url: 'https://catalogo-onlineapi.herokuapp.com/api/files',
            data: data,
            header: {
...
export const deleteFileAction = (id) => async (dispatch, getState) => {
    try {
        const res = await axios.delete(`https://catalogo-onlineapi.herokuapp.com/api/files/${id}`);
        getFilesAction();
        dispatch({
